...
data class EditCampaignRequest (
    ...
    fun canBeMerged(date: Int, campaign: Campaign): Boolean {
        return if (date >= campaign.startDate) {
            (impressionsLimit == campaign.impressionsLimit) and (clicksLimit == campaign.clicksLimit) and
                    (startDate == campaign.startDate) and (endDate == campaign.endDate)
        } else {
